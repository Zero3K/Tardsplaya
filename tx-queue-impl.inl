...
QCS_INLINE auto qcstudio::tx_write_t<QTYPE>::write(const FIRST& _first, REST... _rest) -> typename std::enable_if<!std::is_pointer<FIRST>::value, bool>::type {
    if (!write(_first)) {
        return false;
    }
    return write(_rest...);
...
QCS_INLINE auto qcstudio::tx_write_t<QTYPE>::imp_write(const void* _buffer, uint64_t _size) -> bool {
    ...
    if (_size > available_space) {
        cached_head_    = reinterpret_cast<std::atomic<uint64_t>*>(&queue_.status_.head_)->load(memory_order_acquire);
        available_space = (cached_head_ - tail_ - 1 + capacity_) & (capacity_ - 1);
        if (_size > available_space) {
            auto current_core = (int)GetCurrentProcessorNumber();
            reinterpret_cast<std::atomic<int32_t>*>(&queue_.status_.producer_core_)->store(current_core, memory_order_relaxed);

            // yield if consumer is in the same cpu

            int consumer_core = reinterpret_cast<std::atomic<int32_t>*>(&queue_.status_.consumer_core_)->load(memory_order_relaxed);
    ...
    auto prev_core = reinterpret_cast<std::atomic<int32_t>*>(&queue_.status_.producer_core_)->load(memory_order_relaxed);
    if (prev_core != -1) {
        reinterpret_cast<std::atomic<int32_t>*>(&queue_.status_.producer_core_)->store(-1, memory_order_relaxed);
    }
...
QCS_INLINE qcstudio::tx_write_t<QTYPE>::~tx_write_t() {
    if (!invalidated_) {
        reinterpret_cast<std::atomic<uint64_t>*>(&queue_.status_.tail_)->store(tail_, memory_order_release);  // TODO: check how to deal with this in IPC we need to use https://learn.microsoft.com/en-us/windows/win32/sync/interlocked-variable-access
    }
}
...
QCS_INLINE qcstudio::tx_read_t<QTYPE>::tx_read_t(QTYPE& _queue) : queue_(_queue) {
    storage_     = queue_.storage_;
    head_        = reinterpret_cast<std::atomic<uint64_t>*>(&queue_.status_.head_)->load(memory_order_relaxed);  // relaxed => no sync required as the head is only modified by the consumer (us)
    cached_tail_ = reinterpret_cast<std::atomic<uint64_t>*>(&queue_.status_.tail_)->load(memory_order_relaxed);  // optimistic guess, "gimme whatever you have". Later we'll sync if required!
    capacity_    = queue_.capacity_;                                                       // copy to favour the data locality
    invalidated_ = !_queue.is_ok();
}
...
QCS_INLINE auto qcstudio::tx_read_t<QTYPE>::imp_read(void* _buffer, uint64_t _size) -> bool {
    ...
    if (_size > available_data) {
        cached_tail_   = reinterpret_cast<std::atomic<uint64_t>*>(&queue_.status_.tail_)->load(memory_order_acquire);
        available_data = (cached_tail_ - head_ + capacity_) & (capacity_ - 1);
        if (_size > available_data) {
            auto current_core = (int)GetCurrentProcessorNumber();
            reinterpret_cast<std::atomic<int32_t>*>(&queue_.status_.consumer_core_)->store(current_core, memory_order_relaxed);

            // yield if producer is in the same cpu

            int producer_core = reinterpret_cast<std::atomic<int32_t>*>(&queue_.status_.producer_core_)->load(memory_order_relaxed);
    ...
    auto prev_core = reinterpret_cast<std::atomic<int32_t>*>(&queue_.status_.consumer_core_)->load(memory_order_relaxed);
    if (prev_core != -1) {
        reinterpret_cast<std::atomic<int32_t>*>(&queue_.status_.consumer_core_)->store(-1, memory_order_relaxed);
    }
...
QCS_INLINE qcstudio::tx_read_t<QTYPE>::~tx_read_t() {
    if (!invalidated_) {
        reinterpret_cast<std::atomic<uint64_t>*>(&queue_.status_.head_)->store(head_, memory_order_release);  // TODO: check how to deal with this in IPC we need to use https://learn.microsoft.com/en-us/windows/win32/sync/interlocked-variable-access
    }
}
