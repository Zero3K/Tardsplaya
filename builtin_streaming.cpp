...
bool BufferAndStreamToBuiltinPlayer(
    ...
    AddDebugLog(L"[BUILTIN_MEMORY] Starting built-in streaming with memory maps for " + channel_name + L", URL=" + playlist_url + L", Quality=" + quality);
    ...
    SimpleBuiltinPlayer* player = nullptr;
    {
        ...
        if (!g_builtinPlayer) {
            ...
            if (!g_builtinPlayer->Initialize(hwndStatus)) {
                AddDebugLog(L"[BUILTIN_MEMORY] Failed to initialize built-in player");
