...
TransportStreamRouter::TransportStreamRouter() {
    ...
    // Initialize MPC buffer stall detection workaround state
    ...
    // Initialize video starvation workaround state
    schedule_video_starvation_ = false;
    video_starvation_start_time_ = std::chrono::steady_clock::now();
    video_starvation_duration_ = std::chrono::milliseconds(300); // 300ms starvation period
    
    // Initialize timing controls
    last_workaround_trigger_time_ = std::chrono::steady_clock::now();
            ...
            for (size_t i = 0; i < segment_urls.size(); ++i) {
                ...
                if (FetchHLSSegment(segment_url, segment_data, &cancel_token)) {
                    ...
                    for (const auto& packet_orig : ts_packets) {
                        ...
                        // Apply MPC workaround if enabled and check if packet should be dropped
                        bool should_drop_packet = false;
                        if (current_config_.enable_mpc_workaround) {
                            should_drop_packet = ApplyMPCWorkaround(packet, has_discontinuities && segments_processed == 0);
                        }
                        
                        // Skip packet if workaround indicates it should be dropped
...
bool TransportStreamRouter::ApplyMPCWorkaround(TSPacket& packet, bool is_discontinuity) {
    if (!is_mpc_player_ || !current_config_.enable_mpc_workaround) return false;
    
    // Check if we should drop video packets during starvation period
    if (ShouldDropVideoPacket(packet)) {
        // Return true to indicate this packet should be dropped
        // This creates the exact condition MPC-HC detects: video buffer empty while audio has data
        return true;
    }
    
    // Handle end of starvation period with discontinuity signaling
    if (schedule_video_starvation_) {
        auto now = std::chrono::steady_clock::now();
        auto elapsed = now - video_starvation_start_time_;
        
        if (elapsed >= video_starvation_duration_) {
            // End starvation period
            schedule_video_starvation_ = false;
            
            // Add discontinuity indicator to first video packet after starvation
            if (packet.is_video_packet) {
                ...
                if (log_callback_) {
                    log_callback_(L"[MPC-WORKAROUND] Video starvation period ended - resuming with discontinuity marker");
                }
            }
        }
    }
    
    return false; // Don't drop this packet
}
...
void TransportStreamRouter::HandleAdTransition(bool entering_ad) {
    ...
    if (is_mpc_player_ && current_config_.enable_mpc_workaround) {
        // Apply workaround when exiting ads (main content resuming)
        // This is when the video buffer freeze typically occurs
        if (!entering_ad && was_in_ad) {
            auto now = std::chrono::steady_clock::now();
            
            // Rate limiting to prevent excessive triggering (minimum 5 seconds between triggers)
            if (now >= last_workaround_trigger_time_ + std::chrono::seconds(5)) {
                if (log_callback_) {
                    log_callback_(L"[MPC-WORKAROUND] Exiting ad segment - triggering video starvation to flush stuck buffer");
                }
                
                TriggerVideoStarvationWorkaround();
                last_workaround_trigger_time_ = now;
            }
        }
...
void TransportStreamRouter::TriggerVideoStarvationWorkaround() {
    // Start a brief video starvation period to create buffer stall condition
    // This mimics the exact condition the MPC-HC patch detects: video buffer empty while audio has data
    schedule_video_starvation_ = true;
    video_starvation_start_time_ = std::chrono::steady_clock::now();
    
    if (log_callback_) {
        log_callback_(L"[MPC-WORKAROUND] Starting " + std::to_wstring(video_starvation_duration_.count()) + L"ms video starvation period");
    }
}

bool TransportStreamRouter::ShouldDropVideoPacket(const TSPacket& packet) {
    if (!schedule_video_starvation_ || !packet.is_video_packet) {
        return false;
    }
    ...
    // Drop video packets during starvation period, but keep audio flowing
    bool should_drop = elapsed < video_starvation_duration_;
    
    if (should_drop && log_callback_) {
        static auto last_log_time = std::chrono::steady_clock::time_point{};
        if (now >= last_log_time + std::chrono::milliseconds(100)) { // Log every 100ms
            log_callback_(L"[MPC-WORKAROUND] Dropping video packet during starvation (elapsed: " + 
                         std::to_wstring(elapsed.count() / 1000000) + L"ms)");
            last_log_time = now;
        }
    }
